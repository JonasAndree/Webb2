 ...
 * 
 * Loader loads FBX file and generates Group representing FBX scene. Requires
 * FBX file to be >= 7.0 and in ASCII or >= 6400 in Binary format Versions lower
 * than this may load but will probably have errors
 * 
 * Needs Support: Morph normals / blend shape normals Animation tracks for morph
 * targets
 * 
 ...
 * http://help.autodesk.com/view/FBX/2017/ENU/?guid=__cpp_ref_index_html (C++
 * SDK reference)
 * 
...
( function () {
	...
	// load a texture specified as a blob or data URI, or via an external URL
	...
	// Also parse the texture map and return any textures associated with the
	// material
	function parseParameters( FBXTree, properties, textureMap, ID, connections ) {
		...
		if ( properties.Diffuse ) {
			...
			// The blender exporter exports diffuse here instead of in
			// properties.Diffuse
		...
		if ( properties.Emissive ) {
			...
			// The blender exporter exports emissive color here instead of in
			// properties.Emissive
		...
		if ( properties.Specular ) {
			...
			// The blender exporter exports specular color here instead of in
			// properties.Specular
	...
	function getTexture( FBXTree, textureMap, id, connections ) {
		...
		// if the texture is a layered texture, just use the first layer and
		// issue a warning
	...
	// Deformer node can contain skinning or Vertex Cache animation data,
	// however only skinning is supported here
	...
	// The top level skeleton node has type 'Skin' and sub nodes have type
	// 'Cluster'
	// Each skin node represents a skeleton and each cluster node represents a
	// bone
	...
	// The top level morph deformer node has type "BlendShape" and sub nodes
	// have type "BlendShapeChannel"
		...
		for ( var i = 0; i < relationships.children.length; i ++ ) {
			...
			targetRelationships.children.forEach( function ( child ) {
				...
				if ( child.relationship === 'DeformPercent' ) {
					...
					// weightCurve = FBXTree.Objects.AnimationCurveNode[
					// weightCurveID ];
	...
	function parseMeshGeometry( FBXTree, relationships, geoNode, deformers ) {
		...
		// TODO: if there is more than one model associated with the geometry,
		// AND the models have
	...
	function genGeometry( FBXTree, geoNode, skeleton, morphTarget, preTransform ) {
		...
		if ( geoInfo.material && geoInfo.material.mappingType !== 'AllSame' ) {
			...
			// Convert the material indices of each vertex into rendering groups
			// on the geometry.
			...
			// case where there are multiple materials but the whole geometry is
			// only
			// using one of them
			if ( geo.groups.length === 0 ) {
	...
	function genBuffers( geoInfo ) {
		...
		geoInfo.vertexIndices.forEach( function ( vertexIndex, polygonVertexIndex ) {
			...
			// a: 0, 1, 3, -3, 2, 3, 5, -5, 4, 5, 7, -7, 6, 7, 1, -1, 1, 7, 5,
			// -4, 6, 0, 2, -5
			// }
			// Negative numbers mark the end of a face - first face here is 0,
			// 1, 3, -3
			// to find index of last vertex bit shift the index: ^ - 1
			if ( vertexIndex < 0 ) {

				vertexIndex = vertexIndex ^ - 1; // equivalent to ( x * -1 )
													// - 1
				endOfFace = true;
	...
	// Generate data for a single face in a geometry. If the face is a quad then
	// split it into 2 tris
	...
	// a morph geometry node is similar to a standard node, and the node is also
	// contained
	// in FBXTree.Objects.Geometry, however it can only have attributes for
	// position, normal
	// and a special attribute Index defining which vertices of the original
	// geometry are affected
	// Normal and position attributes only have data for the vertices that are
	...
	function genMorphGeometry( parentGeo, parentGeoNode, morphGeoNode, preTransform ) {
		...
		for ( var i = 0; i < indices.length; i ++ ) {
			...
			// FBX format uses blend shapes rather than morph targets. This can
			// be converted
			// by additively combining the blend shape positions with the
			// original geometry's positions
	...
	// Parse normal from FBXTree.Objects.Geometry.LayerElementNormal if it
	// exists
	...
	// Parse Vertex Colors from FBXTree.Objects.Geometry.LayerElementColor if it
	// exists
	...
	// Parse mapping and material data in
	// FBXTree.Objects.Geometry.LayerElementMaterial if it exists
	function getMaterials( MaterialNode ) {
		...
		// Since materials are stored as indices, there's a bit of a mismatch
		// between FBX and what
		// we expect.So we create an intermediate buffer that points to the
		// index in the buffer,
			...
			if ( ! model ) {
				...
				switch ( node.attrType ) {
					...
					case 'NurbsCurve':
						...
						break;
					case 'LimbNode': // usually associated with a Bone,
										// however if a Bone was not created
										// we'll make a Group instead
					case 'Null':
					default:
			...
			for ( var ID in skeletons ) {
				...
				skeleton.rawBones.forEach( function ( rawBone, i ) {
					...
					if ( rawBone.ID === parent.ID ) {
						...
						// set name and id here - otherwise in cases where
						// "subBone" is created it will not have a name / id
						bone.name = THREE.PropertyBinding.sanitizeNodeName( name );
						bone.ID = id;

						skeleton.bones[ i ] = bone;

						// In cases where a bone is shared between multiple
						// meshes
						// duplicate the bone here and and it as a child of the
						// first bone
						if ( subBone !== null ) {
	...
	function createLight( FBXTree, relationships ) {
		...
		if ( lightAttribute === undefined ) {
			...
			// TODO: could this be calculated linearly from FarAttenuationStart
			...
			switch ( type ) {
				...
				case 2: // Spot
					...
					if ( lightAttribute.OuterAngle !== undefined ) {
						...
						// TODO: this is not correct - FBX calculates outer and
						// inner angle in degrees
						// with OuterAngle > InnerAngle && OuterAngle <= Math.PI
						// while three.js uses a penumbra between (0, 1) to
	...
	// Parse ambient color in FBXTree.GlobalSettings - if it's not set to black
	// (default), create an ambient light
	...
	function setModelTransforms( FBXTree, model, modelNode ) {
		...
		if ( 'RotationOrder' in modelNode ) {
			...
			if ( value > 0 && value < 6 ) {
				...
				// Note: Euler order other than XYZ is currently not supported,
				// so just display a warning for now
	...
	// each AnimationCurveNode holds data for an animation transform for a model
	// (e.g. left arm rotation )
	...
	// previously parsed AnimationCurveNodes. Each AnimationCurve holds data for
	// a single animated
	...
	// parse nodes in FBXTree.Objects.AnimationLayer. Each layers holds
	// references
	// to various AnimationCurveNodes and is referenced by an AnimationStack
	// node
	// note: theoretically a stack can have multiple layers, however in practice
	// there always seems to be one per stack
			...
			if ( connection !== undefined ) {
				...
				children.forEach( function ( child, i ) {
					...
					if ( curveNodesMap.has( child.ID ) ) {
						...
						// check that the curves are defined for at least one
						// axis, otherwise ignore the curveNode
						if ( curveNode.curves.x !== undefined || curveNode.curves.y !== undefined || curveNode.curves.z !== undefined ) {
							...
							if ( layerCurveNodes[ i ] === undefined ) {
								...
								// if the animated model is pre rotated, we'll
								// have to apply the pre rotations to every
	...
	// parse nodes in FBXTree.Objects.AnimationStack. These are the top level
	...
	function parseAnimStacks( FBXTree, connections, layersMap ) {
		...
		for ( var nodeID in rawStacks ) {
			...
			if ( children.length > 1 ) {
				...
				// it seems like stacks will always be associated with a single
				// layer. But just in case there are files
				// where there are multiple layers per stack, we'll display a
				// warning
	...
	// take raw animation data from parseAnimations and connect it up to the
	// loaded models
	...
	// These will be converted to quaternions which don't support values greater
	// than
	...
	Object.assign( TextParser.prototype, {
		...
		parse: function ( text ) {
			...
			split.forEach( function ( line, i ) {
				...
				if ( matchBeginning ) {
					...
					// large arrays are split over multiple lines terminated
					// with a ',' character
					// if this is encountered the line needs to be joined to the
					// previous line
		...
		parseNodeProperty: function ( line, property, contentLine ) {
			...
			// convert string to array, unless it ends in ',' in which case more
			// will be added to it
		...
		parseNodePropertyContinued: function ( line ) {
			...
			// if the line doesn't end in ',' we have reached the end of the
			// property value
	...
	Object.assign( BinaryParser.prototype, {
		...
		parseNode: function ( reader, version ) {
			...
			// note: do not remove this even if you get a linter warning as it
			...
			// Regards the first three elements in propertyList as id, attrName,
			// and attrType
			...
			// like (name, 0) set or (name2, [0, 1, 2]) set of {name: 0, name2:
			// [0, 1, 2]}
			...
			node.propertyList = propertyList; // raw property list used by
												// parent
		...
		parseSubNode: function ( name, node, subNode ) {
			...
			if ( subNode.singleProperty === true ) {
				...
				subNode.propertyList.forEach( function ( property, i ) {
					...
					// first Connection is FBX type (OO, OP, etc.). We'll
					// discard these
		...
		parseProperty: function ( reader ) {
			...
			switch ( type ) {
				...
				case 'l':
					...
					var encoding = reader.getUint32(); // 0: non-compressed, 1:
														// compressed
					...
					var inflate = new Zlib.Inflate( new Uint8Array( reader.getArrayBuffer( compressedLength ) ) ); // eslint-disable-line
																													// no-undef
	...
	Object.assign( BinaryReader.prototype, {
		...
		// There's a possibility that this method returns wrong value if the
		// value
		// is out of the range between Number.MAX_SAFE_INTEGER and
	...
	// FBXTree holds a representation of the FBX data, returned by the
	// TextParser ( FBX ASCII format)
